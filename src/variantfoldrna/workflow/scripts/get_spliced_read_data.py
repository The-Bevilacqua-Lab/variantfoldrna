...
def get_cdna(hgvs, flank, genes, transcript):
    ...
    if five_prime_test(hgvs, 1, flank) and three_prime_test(hgvs, length, flank):
        return genes[f"transcript:{transcript}"][
            hgvs - flank - 2 : hgvs + flank - 1
        ].seq
    else:
        return None
        ...
        if model:
            ...
            if f"transcript:{predictions.iloc[i][feature]}" in cds_dict:
                if cds_dict[f"transcript:{predictions.iloc[i][feature]}"] != "None":
                    cds_start = cds_dict[f"transcript:{predictions.iloc[i][feature]}"][
                        0
                    ]
                ...
                if "offset" not in parsed:
                    ...
                    seq = get_cdna(
                        position, int(args.flank), genes, predictions.iloc[i][feature]
                    )
                    ...
                    if seq:
                        ...
                        if strand == "-1":
                            reference = compelement_dna(
                                predictions.iloc[i]["REF_ALLELE"]
                            )
                        ...
                        chrom_position = int(
                            predictions.iloc[i]["#Location"].split(":")[1]
                        )
                        ...
                        if snp_seq == alternative:
                            ...
                            fn.write(
                                f'{chrom}\t{chrom_position}\t{position - 1}\t{ref}\t{alt}\t{flank_left}\t{flank_right}\t{predictions.iloc[i][feature]}\tMATCHED_ALT\t{predictions.iloc[i]["Consequence"]}\t{predictions.iloc[i]["STRAND"]}\n'
                            )
                        ...
                        # Check to see if the SNP matches the reference allele
                            ...
                            flank = flank_left + ref + flank_right
                            fn.write(
                                f'{chrom}\t{chrom_position}\t{position - 1}\t{ref}\t{alt}\t{flank_left}\t{flank_right}\t{predictions.iloc[i][feature]}\tMATCHED_REF\t{predictions.iloc[i]["Consequence"]}\t{predictions.iloc[i]["STRAND"]}\n'
                            )
                        ...
                        # If the SNP does not match the reference or alternative allele, then we skip it
                            ...
                            alt = reference
                            no_match.write(
                                f"{chrom}\t{chrom_position}\t{position - 1}\t{ref}\t{alt}\t{flank_left}\t{flank_right}\t{predictions.iloc[i][feature]}\n"
                            )
    no_match.close()
    fn.close()
