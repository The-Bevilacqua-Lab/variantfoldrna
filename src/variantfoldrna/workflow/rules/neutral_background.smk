...
rule get_bedtools_genome_file:
    ...
    shell:
        "python3 {src_dir}/../variantfoldrna/envs/workflow/scripts/get_chrom_lengths.py --ref-genome {params.ref} --output {output}"
...
rule trim_intergenic_regions:
    ...
    singularity:
        "docker://kjkirven/process_seq"
    shell:
        "python3 {src_dir}/../variantfoldrna/envs/workflow/scripts/trim_intergenic_regions.py --input {input.gene_model} --output {output}"
...
rule extract_flank_sequences_null:
    ...
    singularity:
        "docker://kjkirven/process_seq"
    shell:
        "python3 {src_dir}/../variantfoldrna/envs/workflow/scripts/extract_flank_sequences_null.py --ref-genome {input.ref} --vcf {input.vcf} --o {output} --flank 50"
...
rule get_bakground_mutation_rate:
    ...
    singularity:
        "docker://kjkirven/process_seq"
    shell:
        "python3 {src_dir}/../variantfoldrna/envs/workflow/scripts/get_background_mutation_rate.py --input {input} --output {output}"
