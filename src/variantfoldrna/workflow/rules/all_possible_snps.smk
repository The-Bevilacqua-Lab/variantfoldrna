...
rule create_vep_other_alts_dir:
    ...
    output:
        directory(f"{config['tmp_dir']}/vep_chunks_all_alts_annotated"),
    shell:
        "mkdir -p {output}"
...
rule vep_other_alts:
    ...
    input:
        ...
        dir=f"{config['tmp_dir']}/vep_chunks_all_alts_annotated",
    output:
        f"{config['tmp_dir']}/vcf_chunks_all_alts_annotated/vcf_no_header_{{i}}_annotated_one_per_line.txt",
    params:
        f"{config['tmp_dir']}/vep_chunks_all_alts_annotated/vcf_no_header_{{i}}_annotated_one_per_line_temp.txt",
    singularity:
...
rule extract_spliced_sequences_generated_alts:
    ...
    input:
        vcf=f"{config['tmp_dir']}/vcf_chunks_all_alts_annotated/vcf_no_header_{{i}}_annotated_one_per_line.txt",
    ...
    output:
        seqs=f"{config['tmp_dir']}/extracted_sequences_all_alts/extracted_seqs_all_alts_{{i}}.txt",
    singularity:
...
rule combine_extracted_sequences_generated_alts:
    ...
    input:
        expand(
            f"{config['tmp_dir']}/extracted_sequences_all_alts/extracted_seqs_all_alts_{{i}}.txt",
            i=range(1, config["chunks"] + 1),
        ),
    message:
        "Combining the extracted sequences for the generated alternative alleles into one file ..."
    output:
        f"{config['tmp_dir']}/extracted_sequences_all_alts/extracted_seqs_all_alts.txt",
    singularity:
        "docker://kjkirven/process_seq"
    shell:
        "cat {input} > {output}"
...
rule remove_duplicates_generated_alts:
    ...
    output:
        f"{config['tmp_dir']}/extracted_sequences_all_alts/extracted_seqs_all_alts_unique.txt",
    singularity:
        "docker://kjkirven/process_seq"
    shell:
...
rule chunk_extracted_sequences_generated_alts:
    ...
    output:
        expand(
            f"{config['tmp_dir']}/extracted_sequences_all_alts/extracted_seqs_all_alts_unique_chunk_{{i}}.txt",
            i=range(1, config["chunks"] + 1),
        ),
    singularity:
        "docker://kjkirven/process_seq"
...
rule run_snpfold_generated_alts:
    ...
    params:
        f"{{temp_deg}}",
    output:
        ribo=f"{config['tmp_dir']}/ribosnitch_chunks_all_alts_{{temp_deg}}/chunk_{{i}}_riboSNitch_{{temp_deg}}.txt",
        error=f"{config['tmp_dir']}/ribosnitch_chunks_all_alts_{{temp_deg}}/chunk_{{i}}_riboSNitch_{{temp_deg}}_error.txt",
    singularity:
...
rule combine_ribosnitch_results_generated_alts:
    ...
    input:
        [
            f"{config['tmp_dir']}/ribosnitch_chunks_all_alts_{{temp_deg}}/chunk_{i}_riboSNitch_{{temp_deg}}.txt"
