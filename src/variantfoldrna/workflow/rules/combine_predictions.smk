...
rule combine_ribosnitch_results_total:
    ...
    input:
        [f"{config['tmp_dir']}/ribosnitch_predictions/combined_ribosnitch_prediction_{tool}_{{temp_deg}}.txt" 
    ...
    params:
        " ".join([f"{config['tmp_dir']}/ribosnitch_predictions/combined_ribosnitch_prediction_{tool}_{{temp_deg}}.txt" 
         for tool in config["ribosnitch_prediction_tool"].lower().split(",")])
    shell:
