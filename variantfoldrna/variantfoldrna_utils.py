...
def bash_builder(output_file, cores, working_directory, singularity_prefix=None, singularity_bind=None,  cluster=False, cluster_info=None, jobs=None):
    '''
    Generates a bash script for running the variantfoldrna pipeline
    '''
    output = open(output_file, "w")
    header = '''#!/bin/bash

# This script runs the variantfoldrna pipeline\n\n'''

    output.write(header)
    output.write("echo 'Running variantfoldrna...'\n\n")

    # Start building the snakemake command:
    if singularity_bind is not None:
        command = f"snakemake -s workflow/variantfoldrna.smk --cores {cores} --use-singularity --singularity-args ' -B {singularity_bind}' --latency-wait 200 --rerun-incomplete "
    else:
        command = f"snakemake -s workflow/variantfoldrna.smk --cores {cores} --use-singularity --singularity-args ' -B {os.path.abspath(working_directory)}' --latency-wait 200 --rerun-incomplete "
