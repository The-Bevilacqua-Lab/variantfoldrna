###########################################################################
# Main file for running the variantfoldrna pipeline. This acts a wrapper that
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run the VariantFoldRNA pipeline")
    parser.add_argument(
        "--vcf",
        dest="vcf",
    ...
    parser.add_argument(
        ...
        dest="variant_class",
        help="Only run the pipeline for a specific category of variants (e.g. missense_variant, synonymous_variant, 5_prime_UTR_variant, 3_prime_UTR_variant, etc.)"
    )
    ...
    bash_builder(f"{output_dir}/variantfoldrna.sh", args.cores, args.out, args.singularity, args. singularity_bind, args.cluster, args.cluster_info, args.jobs)
  
    prGreen("All Done!\n")
    prCyan("To run the VariantFoldRNA pipeline, run the following commands:")
    prCyan("   cd {}".format(output_dir))
    prCyan("   bash variantfoldrna.sh\n")
    prCyan("\n Thank you for using VariantFoldRNA!")
