...
func main() {
	...
	if *vcfPath == "" || *refSeqsPath == "" || *flankLength == 0 || *cdsPosPath == "" || *gffreadPath == "" || *outputPath == "" {
		flag.PrintDefaults()
		os.Exit(1)
	}
	...
	for scanner.Scan() {
		...
		seq := getCDNA(fields[4], *flankLength, *refSeqsPath, fields[5], strand)
...
func getCDNA(transcriptID string, flankLength int, seqPath string, cdsPos string, strand int) string {
	...
	if strand == -1 {
		cmd := exec.Command("samtools", "faidx", seqPath, fmt.Sprintf("transcript:%s:%d-%d", transcriptID, pos-flankLength, pos+flankLength))
