...
rule chunk_vcf:
    ...
    shell:
        f"python3 workflow/scripts/chunk_vcf.py --input {{input.vcf}} --dir {config['working_directory']}/{config['out_name']}/temp --vcf-header {{input.header}} --chunk-total {config['chunks']} > {{log}}"
