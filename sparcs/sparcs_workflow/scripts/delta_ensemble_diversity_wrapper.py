...
def get_delta_ensemble_diversity(ref_seq, alt_seq, temp):
    ...
    try:
        ref = subprocess.run(
            ["RNAfold", "-p", ref_fn, "-T", temp, "--noPS"], stdout=subprocess.PIPE
        )
        alt = subprocess.run(
            ["RNAfold", "-p", alt_fn, "-T", temp,"--noPS"], stdout=subprocess.PIPE
        )
    except:
        return "Error"
...
if __name__ == "__main__":
    # Parse the arguments
    parser = argparse.ArgumentParser(description="Determine delta ensemble diversity")
