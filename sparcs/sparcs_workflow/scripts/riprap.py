...
def make_fa_file(inputfilename,outputfilepath):
    ...
    ofile=file(outputfilepath+'wt_mut_allseq.fa','w')
    for line in ifile:
...
def runRNAfold(fafile,dppsfilepath,temp):
    ...
    if os.path.isfile(fafile):
        ...
        commandline1='RNAfold -p -T ' + str(temp) + ' < '+fafile+' > '+dppsfilepath+'rnafold.out'
    ...
    if os.path.isfile(fafile):
        ...
        while 1:
            ...
            if line.startswith(">"):
                ...
                cmd = "partition -T {0} {1} {2}".format(str(temp),single_fasta,single_pfs)
...
def Getmedian_fc(ppm1,ppm2):
    ...
    dis=max((averagePP1+0.001)/(float(averagePP2)+0.001),(averagePP2+0.001)/(float(averagePP1)+0.001))
    return(dis)
...
def find_region(ppv1,ppv2,Pos,minW):
    ...
    for LB in range(minHW,(Ldis+1)):
        for RB in range(minHW,(Rdis+1)):
            p_value_new=two_sample_ks_test(ppv1[(int(Pos)-LB-1):(int(Pos)+RB)],ppv2[(int(Pos)-LB-1):(int(Pos)+RB)])
...
def Get_all_score(ppmfilename1,ppmfilename2,Pos,length_seq,minW,fr,LBin,RBin):
    ...
    if fr == 0:
        ...
        score , windows , windowe, p_value, score_array =find_region_free(pp1,pp2,Pos,minW)
    elif fr == 2:
        score , windows , windowe, p_value, score_array = find_region_fix(pp1,pp2,LBin,RBin,minW)
...
def output_score_for_snv(input_filename,path_to_ppmfile_folder,outfile_prefix,minW,fr,LBin,RBin,save):
    ...
    for lines in ifile:
        ...
        if os.path.isfile(ppmfilename1) and os.path.isfile(ppmfilename2):
            myseqfeature,score_array=Get_all_score(ppmfilename1,ppmfilename2,Pos,length_seq,minW,fr,LBin,RBin)
...
def Get_all(inputfilename,outfile_prefix,fold_type,minW,fr,LBin,RBin,save,temp):
    ...
    if fold_type == 1: ## RNAfold
        ...
        while not os.path.exists(last_ppm):
            time.sleep(1)
        output_score_for_snv(inputfilename,ppm_path,outfile_prefix,minW,fr,LBin,RBin,save)
...
def garparser():
    argparser = argparse.ArgumentParser(description = "Riprap: Predicting RNA structural disruptions induced by single nucleotide variants\nVersion: 1.0\n", formatter_class = RawTextHelpFormatter)
    argparser.add_argument("-i","--input",dest = "SNV_file",type = str, required = True, help = "Input SNV file with fullpath. Please refer to the webpage for the file format.")
    argparser.add_argument("-o","--outpref",dest = "out_pref",type = str, required = True, help = "Input the prefix of the output file with fullpath to store the output table of Riprap.")
    argparser.add_argument("-t","--foldtype",dest = "fold_type",type = str,required=False,default="1",help = "Please provide the folding algorithm you want to use. 1: RNAfold; 2: RNAstructure; 3: Mfold (UNAfold); or 0: No folding.(Used when you have existing files). If not provided, RNAfold will be performed.")
    argparser.add_argument("-w","--minwin",dest = "minW",type = int,required=False,default=3,help = "Please provide the minimum spanning window size (nt) to select the structure disrupted region. The default minimum window size is 3nt.")
    argparser.add_argument("-f","--wintype",dest = "fr",type = int,required=False,default=0,help = "Please provide the type of spanning window. There are three types of windows available in Riprap: 0: spanning window that include the SNV position; 1: freely spanning window along the entire sequence; 2: spanning window within a fixed region. The first type of window is restricted around the SNV position to select the structure disrupted region, which is the recommended and default option. The second type of window does not have to include the SNV site in searching, which results in longer running time. The last type of window requires the user to provide the start and end site of the interested region on the sequence, which is NOT recommended unless the user know what they are doing. The default is 0, the first type of window.")
    argparser.add_argument("-l","--LBin",dest = "LBin",type = int,required=False,default=0,help = "Please provide the index of the left boundary of the pre-selected region. The default is 0, which is the start site of the sequence. Only use this option when you are interested in a specific region with known biology. This option is not recommended.")
    argparser.add_argument("-r","--RBin",dest = "RBin",type = int,required=False,default=10,help = "Please provide the index of the right boundary of the pre-selected region. The default is 10, which means the 10th position of the sequence. Only use this option when you are interested in a specific region with known biology. This option is not recommended.")
    argparser.add_argument("-s","--save",dest = "save",type = str, required = False, default="no",help = "Whether save the intermediate files. Input yes or no, and default is no.")
    argparser.add_argument("-T", "--temperture", dest = "temp", type = float, required = False, default = 37.0, help = "The temperature for RNA folding. The default is 37.0.")
