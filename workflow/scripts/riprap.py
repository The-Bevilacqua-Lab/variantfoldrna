...
def runRNAfold(fafile, dppsfilepath, temp):
    ...
    if os.path.isfile(fafile):
        ...
        commandline1 = f"RNAfold -p -T {temp} < {fafile} > rnafold.out"
    ...
    if os.path.isfile(fafile):
        ...
        while 1:
            ...
            if line.startswith(">"):
                ...
                cmd = f"partition -T {temp} {single_fasta} {single_pfs}"
...
def Get_all_score(ppmfilename1, ppmfilename2, Pos, length_seq, minW):
...
def output_score_for_snv(
    input_filename, path_to_ppmfile_folder, outfile_prefix, minW, save
):
    ...
    for lines in ifile:
        ...
        if os.path.isfile(ppmfilename1) and os.path.isfile(ppmfilename2):
            myseqfeature, score_array = Get_all_score(
                ppmfilename1, ppmfilename2, Pos, length_seq, minW
            )
...
def Get_all(inputfilename, outfile_prefix, fold_type, minW, save, temperature):
    ...
    if fold_type == 1:  ## RNAfold
        last_ppm = Get_all_prefiles(inputfilename, fa_path, "RNAfold", ppm_path, temperature)
        while not os.path.exists(last_ppm):
            time.sleep(1)
        output_score_for_snv(inputfilename, ppm_path, outfile_prefix, minW, save)
    elif fold_type == 2:  ## RNAstructure
        Get_all_prefiles(inputfilename, fa_path, "RNAstructure", ppm_path, temperature)
        output_score_for_snv(inputfilename, ppm_path, outfile_prefix, minW, save)
    elif fold_type == 3:  ## UNAfold
        Get_all_prefiles(inputfilename, fa_path, "UNAfold", ppm_path)
        output_score_for_snv(inputfilename, ppm_path, outfile_prefix, minW, save)
...
def garparser():
    ...
    argparser.add_argument(
        ...
        required=False,
        default="37.0",
        help="Temperature for structure prediction. Default is 37C.",
    )
...
def RUN():
    ...
    print("Starting...")
    Get_all(inputfile, output_pre, fold_type, minW, save, temperature)
    print("Finished!")
